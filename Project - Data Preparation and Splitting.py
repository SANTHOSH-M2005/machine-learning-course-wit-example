...
# Normalization
minmax = MinMaxScaler()
X_normalized = minmax.fit_transform(X.select_dtypes(include='number'))               # <-- Fill: transform X numeric columns
print("\nAfter Normalization:\n", X_normalized[:5])

# Standardization
scaler = StandardScaler()
X_standardized =  scaler.fit_transform(X.select_dtypes(include='number'))               # <-- Fill: transform X numeric columns
print("\nAfter Standardization:\n", X_standardized[:5])

# Step 5: Train-Test Split
X_train, X_test, y_train, y_test = train_test_split(X,df["Promotion_Eligibility"],test_size=0.2,random_state=42)  # <-- Fill: use train_test_split
